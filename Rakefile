...
# must match the wrapper produced by _includes/icon.html
def icon_span(name)
  svg = File.read(File.join(ICONS_DIR, "#{name}.svg"))
  %Q(<span class="icon fa-#{name}" aria-hidden="true">#{svg}</span>)
end
  ...
  namespace :generate do
    namespace :javascript do
      ...
      task :inline_icons do
        ...
        out = src.gsub(placeholder){ icon_span($1).to_json }
